...
Game::Game(GameApp* parent): GameState(parent) 
{
	...
	mGameType = TEAM;
	// Online mode and Elimination leave Infection rules disabled unless explicitly selected.
...
void Game::Update(float dt)
{
	...
	// Process all delayed infections independently so multiple bots can die in one frame.
	for (int i=mPendingInfectionRespawns.size()-1; i>=0; i--) {
		mInfectionRespawnTimers[i] -= dt*mTimeMultiplier;
		if (mInfectionRespawnTimers[i] <= 0.0f) {
			Person* victim = mPendingInfectionRespawns[i];
			mPendingInfectionRespawns.erase(mPendingInfectionRespawns.begin()+i);
			...
			victim->mInvincibleTime = 1500.0f;
			if (mRespawnStyle == RESPAWN_INPLACE) {
				victim->Teleport(victim->mX,victim->mY);
			}
			else {
				int spawnIndex = rand()%mMap->mNumTs;
				victim->Teleport(mMap->mTSpawns[spawnIndex]->x,mMap->mTSpawns[spawnIndex]->y);
			}
			if (victim == mPlayer) {
				mSpec = victim;
				mSpecState = NONE;
				mPlayer->mTeam = T;
				mPlayer->mOriginalTeam = CT;
			}
		}
	}
	// Only Infection needs the delayed conversion count and infection-specific win check.
	if (mIsInfectionMode && mGameType == TEAM && !mIsOnline && mPendingInfectionRespawns.empty()) {
		mNumRemainingCTs = 0;
		mNumRemainingTs = 0;
		for (unsigned int i=0; i<mPeople.size(); i++) {
			if (mPeople[i]->mState == DEAD) continue;
			if (mPeople[i]->mTeam == CT) mNumRemainingCTs++;
			else if (mPeople[i]->mTeam == T) mNumRemainingTs++;
		}
		if (mWinner == NONE && (mNumRemainingCTs == 0 || mNumRemainingTs == 0)) {
			if (mNumRemainingCTs == 0) {
				mWinner = T;
				mNumTWins++;
				gSfxManager->PlaySample(gRoundEndSounds[T]);
				mTimeMultiplier = 0.33f;
			}
			else if (mNumRemainingTs == 0) {
				mWinner = CT;
				mNumCTWins++;
...
void Game::Render()
{
	...
	**/
	...
	for(unsigned int i=0; i<mPeople.size(); i++)
	{
		...
		else if (mPeople[i]->mHasCorpse) {
			mPeople[i]->RenderCorpse(dx,dy);
		}
	}
	for(unsigned int i=0; i<mGunObjects.size(); i++)
	{
	...
	if (mEngine->GetButtonState(PSP_CTRL_SELECT) || mIsMapChanging) {
		...
		if (mGameType == TEAM) {
			...
			scoreboardTs = 0;
			for (unsigned int i=0; i<mPeople.size(); i++) {
				if (mPeople[i]->mTeam == CT) scoreboardCTs++;
				else if (mPeople[i]->mTeam == T) scoreboardTs++;
			}
		}

		int pages1 = ceil(max2(scoreboardCTs,scoreboardTs)/4.0f);
		if (mGameType == FFA) {
			pages1 = ceil((scoreboardCTs+scoreboardTs)/10.0f);
		}
		if (pages1 == 0) pages1 = 1;
		int pages2 = ceil((mPeople.size()-(scoreboardCTs+scoreboardTs))/10.0f);
		...
		if (mScoreState < pages1) {
			if (mGameType == FFA) {
				gFont->SetColor(ARGB(255,255,255,255));
				char buffer[128];
				sprintf(buffer,"Players (%d)",(scoreboardCTs+scoreboardTs));
			...
			char buffer[128];
			sprintf(buffer,"Spectators (%d)",mPeople.size()-(scoreboardCTs+scoreboardTs));
			gFont->DrawString(buffer, 55.0f, y, JGETEXT_LEFT);
	...
	if (mGuiController != NULL && mGuiController->IsActive())
	{
		...
		// Pause menus use four 25-pixel rows, including the optional online admin row.
...
void Game::UpdateScores(Person* attacker, Person* victim, Gun* weapon) {
	...
	if (mGameType == FFA || mGameType == CTF) {
		...
		if (mIsInfectionMode && !mIsOnline && victim->mOriginalTeam == CT && victim->mTeam == CT) {
			...
			infectedRespawn = true;
			victim->LeaveCorpse();
			mPendingInfectionRespawns.push_back(victim);
			mInfectionRespawnTimers.push_back(mInfectionRespawnDelay*1000.0f);
		}
		if (!infectedRespawn) {
			if (victim->mTeam == CT) {
				mNumRemainingCTs--;
		...
		mNumRemainingCTs = livingCTs;
		mNumRemainingTs = livingTs;
		if (!mIsOnline && mWinner == NONE && mPendingInfectionRespawns.empty()) {
			if (mNumRemainingTs == 0 || mNumRemainingCTs == 0) {
				if (mNumRemainingCTs == 0) {
					mWinner = T;
					mNumTWins++;
					gSfxManager->PlaySample(gRoundEndSounds[T]);
